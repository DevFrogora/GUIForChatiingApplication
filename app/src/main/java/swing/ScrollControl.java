...
class ScrollControl implements AdjustmentListener {
    ...
    public void checkScrollBar(AdjustmentEvent e) {
        ...
        if (previousExtent != extent) {
            ...
            if (extent < previousExtent && caret.getUpdatePolicy() == DefaultCaret.UPDATE_WHEN_ON_EDT) {
    ...
    public JPanel createAndShowUI(JButton btn_send_emoji, AutoreplaceSmiles textFieldMsg, JLayeredPane layeredPane) {
                ...
                if (arg0.getKeyCode() == 10) {
                    try {
                        ...
                        kit.insertHTML(doc, doc.getLength(),
                                "<font color='red'><u><b>" + username + "<b></u></font> : "
                                        + "<font  style='background-color:#000000; color:#ffffff; font-size: 6px;'>"
                                        + now.toString() + "</font><br>" + makeStringToSend(textFieldMsg.getText()),
                                0, 0, null);
        ...
        emojipanelcustomclass.setDataOfTransferedCaret(textComponent, globcaret, kit, doc, emojipanelcustomclass,
                center, username);
        ...
        textPane.addCaretListener(new CaretListener() {
            ...
            public void caretUpdate(CaretEvent e) {
                if (e.getMark() == e.getDot()) {
                    ...
                    while (index >= 0) {
                        try {
                            // Object o = hl.addHighlight(index, index + pattern.length(),
                            // DefaultHighlighter.DefaultPainter);
                            Object o = hl.addHighlight(index, index + pattern.length(),
                                    new DefaultHighlighter.DefaultHighlightPainter(randomColor));
    ...
    String makeStringToSend(String textpanestr) {
        ...
        for (int i = 0; i < emojinamearr.length; i++) {
            while (replaceString.contains(emojinamearr[i])) {
                replaceString = replaceString.replace(emojinamearr[i],
                        "<img src=" + emojiurlarr[i] + " alt=\"smileBlob\" width=\"30\" height=\"20\">");
            }
        }
